...
const WaveNotice = ({ currentUser }) => {
  ...
  const fetchData = async () => {
    const favRes = await authFetch(`${API_BASE_URL}/api/favorites/${currentUser.id}`);
  ...
  const handleDelete = async (fav) => {
    ...
    try {
      await authFetch(`${API_BASE_URL}/api/notifications/${fav.id}`, { method: "DELETE" });
      await fetchData();
    } catch {
      alert("削除に失敗しました");
    }
  ...
  return (
    <div style={{ maxWidth: "500px", margin: "0 auto", padding: "20px" }}>
      ...
      {/* スポット一覧 */}
      {favorites.length === 0 ? (
        <p style={{ color: "#888", fontSize: "0.9rem" }}>お気に入りポイントがありません</p>
      ) : (
            ...
            return (
              ...
              >
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <div style={{ fontWeight: "bold", fontSize: "0.95rem" }}>{fav.point_name}</div>
                    <div style={{ fontSize: "0.85rem", color: s ? "#36A2EB" : "#888", marginTop: "2px" }}>
                      {s ? `🔔 毎日 ${s.notify_time}` : "🔕 未設定"}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: "6px" }}>
                    <button
                      onClick={() => isEditing ? setEditingId(null) : startEdit(fav)}
                      style={{
                        padding: "4px 12px", borderRadius: "4px",
                        border: "1px solid #36A2EB", background: "white",
                        color: "#36A2EB", cursor: "pointer", fontSize: "0.85rem",
                      }}
                    >
                      {s ? "変更" : "設定"}
                    </button>
                ...
                {isEditing && (
                  <div style={{ marginTop: "10px", display: "flex", gap: "6px", alignItems: "center" }}>
                    ...
                    <button
                      ...
                      className="editBtn"
                      style={{
                        border: "1px solid #aaa", background: "white", color: "#555",
                      }}
                    >
                      キャンセル
                    </button>
        ...
        {linked ? (
          ...
          <>
            ...
            <div style={{
              ...
              letterSpacing: "0.5rem", padding: "12px",
